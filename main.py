...
async def invite_voice_command(
    ...
    try:
        ...
        # Формируем embed для приглашения (ПЕРВОЕ СООБЩЕНИЕ)
        ...
        invite_embed.add_field(
            ...
            value="*Смотри следующее сообщение ↓*",
            inline=False
        )
        ...
        try:
            # Отправляем ПЕРВОЕ сообщение - приглашение
            await пользователь.send(embed=invite_embed)
            
            # Отправляем ВТОРОЕ сообщение - чистую ссылку
            link_message = await пользователь.send(f"**🔗 Ссылка для подключения к голосовому каналу:**\n{voice_link}")
